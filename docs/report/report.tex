...
from 0.5382 to 0.5965 on a separate 10-query test set.
\end{abstract}
...
\href{https://github.com/julilili42/labeling-lab}{crawl-model training}
(\texttt{LABELING-COMMIT}) are maintained in separate repositories.
...
likely to lead to Tübingen-related content. It follows the Robots Exclusion
Protocol \citep{rfc9309} and filters fetched pages using the HTML
\texttt{lang} attribute. Multiple
...
signals from human judgments and Qwen-generated labels \citep{qwen25}. The resulting
lightweight student models keep the collection compact while favoring relevant,
useful pages.
...
\textsc{PageVerdict}, Qwen~2.5:7B provides automatically generated labels alongside human
judgments. German pages receive negative labels regardless of topical
...
scoring. It is serialized with MessagePack. On the complete 8,595-document
index, MessagePack was 37\% smaller than compact JSON (46.0 vs.\ 73.6\,MB) and
had a 2.40-times lower median deserialization time over 20 runs (307 vs.\
736\,ms). For term $t$, document $d$, and fields $f$, BM25F computes
\[
...
repeat of the sweep after the final language safeguard selected the same
weights.
...
the four ablation variants and all eleven fusion weights. A single assessor
grades each query--URL pair from 0 (irrelevant) to 3 (highly relevant) using
URL, title, and snippet. We select the design and
...
development. After the final language safeguard was applied, we reran the
frozen variants, repeated the weight sweep, and judged newly surfaced pooled
results. The repeated sweep confirmed rather than changed the frozen weights.
The test pool contains 308 judgments and the development pool 788. nDCG@20,
...
\midrule
Dev. (20) & BM25F    & no  & no  & 0.5250 & 0.5412 & 0.9350 &  1.79 \\
 & BM25F + proximity & yes & no  & 0.5042 & 0.5304 & 0.9017 & 15.61 \\
 & BM25F + semantic  & no  & yes & \textbf{0.5953} & \textbf{0.6619} &
  \textbf{0.9375} & 57.37 \\
 & Full              & yes & yes & 0.5849 & 0.6543 & \textbf{0.9375} & 57.22 \\
\midrule
Test (10) & BM25F    & no  & no  & 0.5382 & 0.5798 & 0.8600 & 1.88 \\
 & BM25F + semantic  & no  & yes & \textbf{0.5965} & \textbf{0.6807} &
  \textbf{0.8700} & 58.44 \\
\bottomrule
\end{tabular}
\end{table*}

Semantic re-ranking improves development nDCG@10 by 0.0702 over BM25F, whereas proximity
...
Batch retrieval returns query ID, rank, URL, and score as TSV.
...
\section{Limitations}
The single-assessor grades use retrieved metadata, and both pools contain only
results produced by the compared systems. The independent test
avoids tuning on reported results, but ten queries remain too few to establish
statistical significance or superiority beyond this collection. It is also
separate from the official five-query course evaluation.
...
Language identification is automated and conservative. Mixed-language pages
may remain indexed, while short pages are retained when too little text is
available for a reliable decision.
